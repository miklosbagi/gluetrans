...
pick_new_gluetun_server() {
    ...
    if ! echo "$gluetun_server_response" | grep -qE '{"outcome":"stopp(ed|ing)"}'; then
        log "bleh, gluetun server response is weird, expected {\"outcome\":\"stopped\"} (for v3.38 or earlier) or {\"outcome\":\"stopping\"} (for v3.39 and later), got $gluetun_server_response"
        return 1
    fi    
